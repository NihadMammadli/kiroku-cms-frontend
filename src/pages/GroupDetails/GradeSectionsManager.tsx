...
const GradeSectionsManager = ({
	syllabusId,
	groupId,
	onBack,
}: GradeSectionsManagerProps) => {
	const [messageApi, contextHolder] = message.useMessage();
	const [activeSection, setActiveSection] = useState<string>("");
	const [gradeModalVisible, setGradeModalVisible] = useState(false);
	const [selectedSection, setSelectedSection] = useState<GradingSection | null>(
		null,
	);
	...
	const { data: gradesData, isLoading: gradesLoading } = useStudentGradesQuery(
		sectionId || 0,
		{},
	);

	const { data: studentReport, isLoading: reportLoading } =
		useStudentGradeReportQuery({
	...
	const handleSaveGrades = async () => {
		...
		const invalidScores = gradeInputs.filter(
			(g) => g.score < 0 || g.score > maxScore,
		);
	...
	const getSectionStats = (_sectionId: number) => {
		...
		const average =
			graded > 0
				? sectionGrades.reduce((sum, g) => sum + Number(g.score), 0) / graded
				: 0;
	...
	const gradeColumns = [
		...
		},
		{
			...
			render: (percentage: number) => (
				<Progress percent={Number(percentage)} size="small" />
			),
		},
		{
			title: "Çəki ilə",
		...
		},
		{
			...
			render: (_: unknown, record: StudentGrade) => (
				<Button
					type="link"
					size="small"
					onClick={() => handleViewReport(record.student)}
				>
					Hesabat
				</Button>
			),
			...
			children: (
				<Card>
					<Space direction="vertical" size="large" style={{ width: "100%" }}>
						...
						{section.description && (
							<Alert message={section.description} type="info" showIcon />
						)}
				...
				{selectedSection && (
					<div>
						<Alert
							message={
								<span>
									Maksimum Bal: <strong>{selectedSection.max_score}</strong> |
									Çəki: <strong>{selectedSection.weight}%</strong>
								</span>
							}
							type="info"
								...
								},
								{
									...
									render: (
										...
										<InputNumber
											...
											onChange={(value) =>
												setGrades({ ...grades, [record.id]: value || 0 })
											}
											style={{ width: "100%" }}
										/>
									),
						...
						<Table
							...
							columns={[
								...
								},
								{
									...
									key: "percentage",
									render: (p: number) => (
										<Progress percent={Number(p)} size="small" />
									),
								},
								{
									title: "Çəkili Bal",
