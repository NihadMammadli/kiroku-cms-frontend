...
const GroupSyllabus = ({ groupId }: GroupSyllabusProps) => {
	...
	const handleModalOk = async () => {
		try {
			...
			if (totalWeight !== 100) {
				messageApi.error(
					`Ümumi çəki 100% olmalıdır (hazırda: ${totalWeight}%)`,
				);
				return;
			}
	...
	const updateSection = (id: string, field: string, value: string | number) => {
		const newSections = formSections.map((s) =>
		...
		return (
			<>
						...
						description={
							<div>
								...
								<p>
									...
									{isTeacher &&
										" Yeni meyar yaratmaq üçün aşağıdakı düyməni klikləyin."}
								</p>
							</div>
						}
					>
				...
				<Modal
					title={
						activeSyllabus
							? "Qiymətləndirmə Meyarını Dəyişdir"
							: "Yeni Qiymətləndirmə Meyarı"
					}
					open={modalVisible}
					onOk={handleModalOk}
									...
									<Row gutter={12}>
										<Col span={12}>
											...
											<Input
												value={section.name}
												onChange={(e) =>
													updateSection(section._id, "name", e.target.value)
												}
										...
										<Col span={6}>
											...
											<InputNumber
												...
												onChange={(value) =>
													updateSection(section._id, "weight", value || 0)
												}
												style={{ width: "100%" }}
											/>
										</Col>
										<Col span={6}>
											...
											<InputNumber
												...
												onChange={(value) =>
													updateSection(section._id, "max_score", value || 100)
												}
												style={{ width: "100%" }}
											/>
										</Col>
